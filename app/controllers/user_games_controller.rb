...
class UserGamesController < ApplicationController
  ...
  def results
    ...
    @filter_matches = @matches.reject do |m|
      current_user.friends.include?(m.user) || current_user.friendships.pluck(:friend_id).include?(m.user.id) || m.user.friendships.pluck(:friend_id).include?(current_user.id)
